...
def is_turbospectrum_support_enabled():
    ...
    if (
        not os.path.exists(turbospectrum_eqwidt_lu)
        or not os.path.exists(turbospectrum_bsyn_lu)
        or not os.path.exists(turbospectrum_babsma_lu)
        or not os.path.exists(turbospectrum_data)
    ):
        return False
    else:
        return True
...
def is_moog_support_enabled():
    ...
    if not os.path.exists(moog_executable) or not os.path.exists(moog_dir):
        return False
    else:
        return True
...
def is_width_support_enabled():
    ...
    if not os.path.exists(width_executable) or not os.path.exists(atmos_dir):
        return False
    else:
        return True
...
def report_progress(current_work_progress, last_reported_progress):
    ...
    return (
        (
            int(current_work_progress) % 10 == 0
            and current_work_progress - last_reported_progress > 10
        )
        or last_reported_progress < 0
        or current_work_progress == 100
    )
...
def __baryvel(datetime, deq=0):
    ...
    dcfel = np.array(
        [
            1.7400353e00, 6.2833195099091e02, 5.2796e-6, 6.2565836e00, 6.2830194572674e02, 
            -2.6180e-6, 4.7199666e00, 8.3997091449254e03, -1.9780e-5, 1.9636505e-1, 
            8.4334662911720e03, -5.6044e-5, 4.1547339e00, 5.2993466764997e01, 5.8845e-6, 
            4.6524223e00, 2.1354275911213e01, 5.6797e-6, 4.2620486e00, 7.5025342197656e00, 
    ...
    ccsel = np.array(
        [ 
            1.675104e-2, -4.179579e-5, -1.260516e-7, 2.220221e-1, 2.809917e-2, 1.852532e-5,
            1.589963e00, 3.418075e-2, 1.430200e-5, 2.994089e00, 2.590824e-2, 4.155840e-6, 
            8.155457e-1, 2.486352e-2, 6.836840e-6, 1.735614e00, 1.763719e-2, 6.370440e-6, 
            1.968564e00, 1.524020e-2, -2.517152e-6, 1.282417e00, 8.703393e-3, 2.289292e-5, 
            2.280820e00, 1.918010e-2, 4.484520e-6, 4.833473e-2, 1.641773e-4, -4.654200e-7, 
            5.589232e-2, -3.455092e-4, -7.388560e-7, 4.634443e-2, -2.658234e-5, 7.757000e-8, 
            8.997041e-3, 6.329728e-6, -1.939256e-9, 2.284178e-2, -9.941590e-5, 6.787400e-8, 
            4.350267e-2, -6.839749e-5, -2.714956e-7, 1.348204e-2, 1.091504e-5, 6.903760e-7, 
    ...
    dcargs = np.array(
        [
            5.0974222e0, -7.8604195454652e2, 3.9584962e0, -5.7533848094674e2, 1.6338070e0, 
    ...
    ccamps = np.array(
        [
            -2.279594e-5, 1.407414e-5, 8.273188e-6, 1.340565e-5, -2.490817e-7, -3.494537e-5, 
            2.860401e-7, 1.289448e-7, 1.627237e-5, -1.823138e-7, 6.593466e-7, 1.322572e-5, 
            9.258695e-6, -4.674248e-7, -3.646275e-7, 1.140767e-5, -2.049792e-5, -4.747930e-6, 
            -2.638763e-6, -1.245408e-7, 9.516893e-6, -2.748894e-6, -1.319381e-6, -4.549908e-6, 
            -1.864821e-7, 7.310990e-6, -1.924710e-6, -8.772849e-7, -3.334143e-6, -1.745256e-7, 
            -2.603449e-6, 7.359472e-6, 3.168357e-6, 1.119056e-6, -1.655307e-7, -3.228859e-6, 
            1.308997e-7, 1.013137e-7, 2.403899e-6, -3.736225e-7, 3.442177e-7, 2.671323e-6, 
            1.832858e-6, -2.394688e-7, -3.478444e-7, 8.702406e-6, -8.421214e-6, -1.372341e-6, 
            -1.455234e-6, -4.998479e-8, -1.488378e-6, -1.251789e-5, 5.226868e-7, -2.049301e-7, 
            0.0e0, -8.043059e-6, -2.991300e-6, 1.473654e-7, -3.154542e-7, 0.0e0, 3.699128e-6, 
            -3.316126e-6, 2.901257e-7, 3.407826e-7, 0.0e0, 2.550120e-6, -1.241123e-6, 9.901116e-8, 
            2.210482e-7, 0.0e0, -6.351059e-7, 2.341650e-6, 1.061492e-6, 2.878231e-7, 0.0e0,
        ]
    )
    ...
    ccsec = np.array(
        [
            1.289600e-6, 5.550147e-1, 2.076942e00, 3.102810e-5, 4.035027e00, 3.525565e-1, 
    ...
    dcargm = np.array(
        [
            5.1679830e0, 8.3286911095275e3, 5.4913150e0, -7.2140632838100e3, 5.9598530e0, 1.5542754389685e4,
        ]
    )
    ...
    ccampm = np.array(
        [
            1.097594e-1, 2.896773e-7, 5.450474e-2, 1.438491e-7, -2.223581e-2, 5.083103e-8, 
...
def calculate_barycentric_velocity_correction(datetime, coordinates, deq=0):
    ...
    ra = ra_hours + ra_minutes / 60.0 + ra_seconds / (60.0 * 60)  # hours
    ra = ra * 360.0 / 24  # degrees
    ra = ra * ((2 * np.pi) / 360.0)  # radians
    dec = dec_degrees + dec_minutes / 60.0 + dec_seconds / (60.0 * 60)  # degrees
    dec = dec * ((2 * np.pi) / 360.0)  # radians

    # Project velocity toward star
    barycentric_vel = (
        dvelb[0] * np.cos(dec) * np.cos(ra)
        + dvelb[1] * np.cos(dec) * np.sin(ra)
        + dvelb[2] * np.sin(dec)
    )  # km/s
    ...
    if isinstance(tz, str) or isinstance(tz, tzinfo):
        ...
        for t in zip(yr, month, day, hr, min, sec, msec):
            ...
            if utcdt is None:
                ...
                utcoffset.append(
                    utcdt.days * 24 + (utcdt.seconds + utcdt.microseconds * 1e-6) / 3600
                )
    else:
        utcoffset = tz
...
def _estimate_vmac_doyle2014(teff, logg, feh):
    ...
    if logg >= 3.5:
        if teff >= 5000:
            # main sequence and subgiants (RGB)
            vmac = (
                3.21 + 2.33e-3 * (teff - t0) + 2e-6 * (teff - t0) ** 2 - 2 * (logg - g0)
            )
        else:
            # main sequence
            vmac = (
...
def estimate_mass_radius(teff, logg, feh):
    ...
    if teff > 38000 or teff < 3120:
        logging.warning(
            "Teff {} is out of the Teff range used to derive the empirical relation".format(
                teff
            )
        )
    if logg > 5.009 or logg < 2.122:
        logging.warning(
            "logg {} is out of the logg range used to derive the empirical relation".format(
                logg
            )
        )
    ...
    if feh > 0.40 or feh < -0.60:
        logging.warning(
            "Metallicity {} is out of the metallicity range used to derive the empirical relation".format(
                feh
            )
        )
