    ...
    * Inspired by PyModelFit UniformCDFKnotSplineModel:
    ...
    def fitData(self, x, y, weights=None):
        ...
        if self.CDF:
            ...
            cdf, xcdf = np.histogram(
                x, bins=max(10, max(2 * self.nknots, int(len(x) / 10)))
            )
            mask = cdf != 0
            cdf, xcdf = cdf[mask], xcdf[np.hstack((True, mask))]
            cdf = np.hstack(
                (0, (1.0 * np.cumsum(cdf)) / np.sum(cdf))
            )  # Multiply by 1.0 to convert to float and have a good division result
        ...
        self.spline = LSQUnivariateSpline(
            x, y, t=self.iknots, k=int(self.degree), w=weights
        )
...
def read_continuum_regions(continuum_regions_filename):
    ...
    continuum_regions = np.array(
        [
            tuple(cont.rstrip("\r\n").split("\t"))
            for cont in open(
                continuum_regions_filename,
            )
        ][1:],
    ...
    if np.any(continuum_regions["wave_top"] - continuum_regions["wave_base"] <= 0):
        logging.error(
            "Continuum regions where wave_base is equal or bigger than wave_top"
        )
...
def write_continuum_regions(continuum_regions, continuum_regions_filename):
    ...
    out.write(
        "\n".join(
            [
                "\t".join(map(str, (cont["wave_base"], cont["wave_top"])))
                for cont in continuum_regions
            ]
        )
    )
    out.close()
...
def __find_max_value_per_wavelength_range(spectrum, base_points, wave_range=1):
    ...
    """
    return __find_a_value_per_wavelength_range(
        spectrum, base_points, wave_range=wave_range, median=False
    )
...
def __find_median_value_per_wavelength_range(spectrum, base_points, wave_range=1):
    ...
    """
    return __find_a_value_per_wavelength_range(
        spectrum, base_points, wave_range=wave_range, median=True
    )


def __find_a_value_per_wavelength_range(
    spectrum, base_points, wave_range=1, median=False
):
    """
    ...
    while wave_base < wave_top:
        positions = np.where(
            (waveobs[base_points] >= wave_base)
            & (waveobs[base_points] < wave_base + wave_step)
        )[0]
        ...
        if len(positions) == 0:
            ...
            if median:
                ...
                sortedi = np.argsort(
                    np.abs(f - np.median(f))
                )  # The smallest is the median (this is the best way to avoid floating-point imprecisions)
...
def __discard_outliers_for_continuum_candidates(spectrum, candidate_base_points, sig=3):
    ...
    flux_diff1 = (
        spectrum["flux"][candidate_base_points][:-1]
        - spectrum["flux"][candidate_base_points][1:]
    ) / (
        spectrum["waveobs"][candidate_base_points][:-1]
        - spectrum["waveobs"][candidate_base_points][1:]
    )
    flux_diff2 = (
        spectrum["flux"][candidate_base_points][1:]
    ...
    flux_diff1_selected, not_outliers1 = interquartile_range_filtering(
        flux_diff1, k=1.5
    )
    flux_diff2_selected, not_outliers2 = interquartile_range_filtering(
        flux_diff2, k=1.5
    )
    outliers = np.logical_or(
        np.logical_not(not_outliers1), np.logical_not(not_outliers2)
    )
...
def __determine_continuum_base_points(
    spectrum, discard_outliers=True, median_wave_range=0.1, max_wave_range=1
):
    """
    ...
    if median_wave_range > 0:
        candidate_base_points = __find_median_value_per_wavelength_range(
            spectrum, candidate_base_points, wave_range=median_wave_range
        )
    if max_wave_range > 0:
        candidate_base_points = __find_max_value_per_wavelength_range(
            spectrum, candidate_base_points, wave_range=max_wave_range
        )
    if discard_outliers:
        candidate_base_points = __discard_outliers_for_continuum_candidates(
            spectrum, candidate_base_points
        )
...
def fit_continuum(
    ...
    ######## Zhecheng Hu Notes ########
    The median filter is aim to avoid noisy peaks and outliers. The max filter is aim to avoid strong lines.
    """
    if independent_regions is not None:
        ...
        wave_step = 0.0001
        xaxis = np.arange(
            spectrum["waveobs"][0], spectrum["waveobs"][-1] + wave_step, wave_step
        )
        ...
        for i, region in enumerate(independent_regions):
            wfilter = np.logical_and(
                spectrum["waveobs"] >= region["wave_base"],
                spectrum["waveobs"] <= region["wave_top"],
            )
            try:
                if len(spectrum[wfilter]) > 10:
                    if template is not None:
                        tfilter = np.logical_and(
                            template["waveobs"] >= region["wave_base"],
                            template["waveobs"] <= region["wave_top"],
                        )
                    ...
                    continuum = __fit_continuum(
                        ...
                        model=model,
                        order=order,
                        automatic_strong_line_detection=automatic_strong_line_detection,
                        strong_line_probability=strong_line_probability,
                        use_errors_for_fitting=use_errors_for_fitting,
                        template=template_tmp,
                    )
                    # Save
                    wfilter = np.logical_and(
                        xaxis >= region["wave_base"], xaxis <= region["wave_top"]
                    )
                    fluxes[np.where(wfilter)[0]] = continuum(xaxis[wfilter])
                    errors[np.where(wfilter)[0]] = continuum.placement_errors(
                        xaxis[wfilter]
                    )
                    num_success += 1
            except:
                print(
                    "Continuum fit failed for segment #",i, "[", region["wave_base"], ",", region["wave_top"], "]",
                )  # fmt: skip
                pass
        if num_success == 0:
        ...
        continuum = interpolate.interp1d(
            xaxis, fluxes, kind="linear", bounds_error=False, fill_value=0.0
        )
        placement_errors = interpolate.interp1d(
            xaxis, errors, kind="linear", bounds_error=False, fill_value=0.0
        )
...
def __clean_outliers(
    spectrum, min_wave, max_wave, wave_step, ignored_regions, probability=0.50
):
    # Resample to have 1 point for each max window instead of N
    wavelengths = np.arange(min_wave, max_wave, wave_step / 2.0)
    ...
    smooth1["flux"] = np.interp(
        smooth1["waveobs"], spectrum["waveobs"][~ignore1], spectrum["flux"][~ignore1]
    )
    ...
    chunk_size = np.max((10.0, 20.0 * wave_step))  # nm
    ...
    for wave_top in [min_wave + chunk_size, min_wave + chunk_size / 2.0]:
        while True:
            wfilter = np.logical_and(
                smooth1["waveobs"] >= wave_top - chunk_size,
                smooth1["waveobs"] < wave_top,
            )
            if np.all(
                np.abs(y[wfilter] - y[wfilter][0]) > 1e-10
            ):  # Avoid arrays with all equal values
                ...
                x_c = sm.add_constant(
                    x[wfilter], prepend=False
                )  # Add a constant (1.0) to have a parameter base
                huber_t = sm.RLM(y[wfilter], x_c, M=sm.robust.norms.HuberT())
                linear_model = huber_t.fit()
                # Accept only if always has been accepted
                sfilter1a[wfilter] = np.logical_and(
                    sfilter1a[wfilter], linear_model.weights > probability
                )  # True equals to "accept region"
    ...
    for wave_top in [min_wave + chunk_size, min_wave + chunk_size / 2.0]:
        while True:
            ...
            if np.all(
                ...
                linear_model = huber_t.fit()
                # Accept only if always has been accepted
                sfilter2a[wfilter] = np.logical_and(
                    sfilter2a[wfilter], linear_model.weights > probability
                )  # True equals to "accept region"
            if np.abs(wave_top - max_wave) < 1.0e-10:
                break
            else:
    ...
    new_spectrum["flux"] = np.interp(
        new_spectrum["waveobs"], spectrum["waveobs"][sfilter], spectrum["flux"][sfilter]
    )
    new_spectrum["err"] = spectrum[
        "err"
    ]  # Keep original errors since the fluxes have not changed (error propagation not needed), only some have been filtered
...
def __clean_ignored_regions(spectrum, ignored_regions):
    ...
    # Resample only fluxes using directly interp instead of ispec.resample_spectrum and
    # choosing NOT to have zeros in the borders
    clean_spectrum["flux"] = np.interp(
        clean_spectrum["waveobs"],
        spectrum["waveobs"][~ignore1],
        spectrum["flux"][~ignore1],
    )
    clean_spectrum["err"] = spectrum[
        "err"
    ]  # Keep original errors since the fluxes have not changed (error propagation not needed), only some have been filtered
...
def __create_gap_regions(spectrum):
    ...
    if len(gap_index) > 1:  # At least two zeros or negative values
        right_side = np.hstack(
            (
                gap_index[np.where(gap_index[1:] - gap_index[:-1] != 1)[0]],
                [gap_index[-1]],
            )
        )
        left_side = np.hstack(
            (
                [gap_index[0]],
                gap_index[np.where(gap_index[1:] - gap_index[:-1] != 1)[0] + 1],
            )
        )
        gap_limits_index = np.transpose(np.vstack((left_side, right_side)))
        gap_regions = np.recarray(
            (len(gap_limits_index),),
            dtype=[
                ("wave_peak", float),
                ("wave_base", float),
                ("wave_top", float),
        ...
        for i, gap_limit in enumerate(gap_limits_index):
            ...
            top = np.min(
                (gap_limit[1] + 1, len(spectrum) - 1)
            )  # One point to the right if possible
            # Wavelengths:
            gap_regions["wave_base"][i] = spectrum["waveobs"][base]
            gap_regions["wave_top"][i] = spectrum["waveobs"][top]
            gap_regions["wave_peak"][i] = (
                gap_regions["wave_base"][i] + gap_regions["wave_top"][i]
            ) / 2.0
            gap_regions["note"][i] = ""
    else:
        # No gaps found
...
def __fit_continuum(
    ...
    class ConstantValue(object):
        ...
        def placement_errors(self, value):
            return (
                value * 0
            )  # The multiplication will return an array or a single value depending on the input
    ...
    if model in ["Splines", "Template"] and nknots is None:
        # One each 5 nm
        nknots = np.max(
            [1, int((np.max(spectrum["waveobs"]) - np.min(spectrum["waveobs"])) / 5.0)]
        )
    if (
        model not in ["Template", "Fixed value"]
        and order == "max+median"
        and median_wave_range <= max_wave_range
    ):
        raise Exception(
            "For 'max+median' order, median_wave_range should be greater than max_wave_rage!"
        )
    if (
        model not in ["Template", "Fixed value"]
        and order == "median+max"
        and median_wave_range >= max_wave_range
    ):
        raise Exception(
            "For 'median+max' order, median_wave_range should be smaller than max_wave_rage!"
        )
    ...
    ####################################################
    # Generate the total ignored regions list:
    # original zeros + regions to be ignored + non-continuum regions
    ####################################################
    # Convert fluxes from regions to be ignored to zeros
    ...
    if continuum_regions is not None:
        ignore2 = np.logical_not(
            create_wavelength_filter(spectrum, regions=continuum_regions)
        )
    ...
    # If no resolution is specified, the spectrum is assumed to be already
    # sampled at the minimum required sampling.
    if from_resolution is not None:
        wave_step = np.min(spectrum["waveobs"]) / from_resolution
        wavelengths = np.arange(
            np.min(spectrum["waveobs"]),
            np.max(spectrum["waveobs"]) + wave_step,
            wave_step,
        )
        resampling_log_message = (
            "Resampling spectrum to wave_step: %.5f nm (R = %i)"
            % (wave_step, from_resolution)
        )
    else:
        spectrum.sort(order="waveobs")
        wave_step = np.median(
            np.abs(spectrum["waveobs"][1:] - spectrum["waveobs"][:-1])
        )
    ...
    if model == "Template":
        ...
        smoothed_continuum_tmp1 = median_filter(
            spectrum["flux"][useful] / template["flux"][useful], med_filter_step
        )
        smoothed_continuum_tmp2 = np.interp(
            template["waveobs"], template["waveobs"][useful], smoothed_continuum_tmp1
        )
    ...
    if len(spectrum["waveobs"]) == len(wavelengths) and np.all(
        np.abs(spectrum["waveobs"] - wavelengths) < 1e-6
    ):
        ...
        resampled_spectrum = resample_spectrum(
            spectrum[~zeros], wavelengths, method="linear", zero_edges=False
        )
    ...
    if order == "max+median":
        ...
        if automatic_strong_line_detection and max_wave_range > 0:
            smooth1, outliers = __clean_outliers(
                smooth1,
                np.min(spectrum["waveobs"]),
                np.max(spectrum["waveobs"]),
                max_wave_range,
                ignored_regions,
                probability=strong_line_probability,
            )
        ...
        # Zhecheng: How many points are there in the median wave range?
        # If it's larger than 1, then we need to select the points around the median
        med_filter_step = int(np.round(median_wave_range / wave_step))
        if med_filter_step > 1:
        ...
        if automatic_strong_line_detection:
            smooth2, outliers = __clean_outliers(
                smooth2,
                np.min(spectrum["waveobs"]),
                np.max(spectrum["waveobs"]),
                max_wave_range,
        ...
        raise Exception(
            "Unknown order (the only valid ones are max+median or median+max)"
        )
    ...
    if last_step > 1:
        ...
        smooth3["err"][more_uncertain] = smooth3["err"][more_uncertain] * 2

    ##### Fit the continuum
    # Resample avoiding zeros and repating the last good value in the borders (not zeros!)
    continuum = resample_spectrum(
        smooth3, spectrum["waveobs"], method="linear", zero_edges=False
    )
    ...
    if model == "Splines":
        ...
        if use_errors_for_fitting and np.any(smooth_err[~ignore1] > 0):
            smooth_weights = 1.0 / smooth_err[~ignore1]
            continuum_model.fitData(
                continuum["waveobs"][~ignore1],
                continuum["flux"][~ignore1],
                weights=smooth_weights,
            )
        else:
            continuum_model.fitData(
                continuum["waveobs"][~ignore1], continuum["flux"][~ignore1]
            )
    else:
        smooth_err = gaussian_filter(continuum["err"], last_step)
        if use_errors_for_fitting and np.any(smooth_err[~ignore1] > 0):
            smooth_weights = 1.0 / smooth_err[~ignore1]
            continuum_model = PolyContinuum(
                np.poly1d(
                    np.polyfit(
                        continuum["waveobs"][~ignore1],
                        continuum["flux"][~ignore1],
    ...
    continuum["err"] += np.abs(
        continuum["flux"] - continuum_model(continuum["waveobs"])
    )
    ...
    continuum_model.placement_errors = interpolate.interp1d(
        continuum["waveobs"],
        continuum["err"],
        kind="linear",
        bounds_error=False,
    ...
    if plot:
        plt.plot(spectrum["waveobs"], spectrum["flux"])
        plt.plot(smooth1["waveobs"], smooth1["flux"])
        plt.plot(smooth2["waveobs"], smooth2["flux"])
        plt.plot(spectrum["waveobs"], continuum_model(spectrum["waveobs"]))
...
def find_continuum(
    ...
    resolution,
    segments=None,
    max_std_continuum=0.002,
    continuum_model=0.95,
    max_continuum_diff=0.01,
    ...
    if segments is None:
        # Use whole spectrum
        segments = np.array(
            [(min_wave, max_wave)], dtype=[("wave_base", float), ("wave_top", float)]
        )
    ...
    for segment in segments:
        ...
        while wave_top < max_limit:
            ...
            wave_filter = (spectrum["waveobs"] >= wave_base) & (
                spectrum["waveobs"] < wave_top
            )
            ...
            if num_measures > 0 and std_flux < max_std_continuum and near_continuum:
                last_accepted = True
                dirty_continuum_regions.append(
                    (wave_base, wave_top, num_measures, mean_flux, std_flux)
                )
            else:
                last_accepted = False
...
def __merge_regions(spectrum, dirty_continuum_regions):
    ...
    while i < len(dirty_continuum_regions) - 2:
        ...
        while (
            dirty_continuum_regions[j + i]["wave_top"]
            == dirty_continuum_regions[j + i + 1]["wave_base"]
        ) and (j < len(dirty_continuum_regions) - 2 - i):
            j += 1
        ...
        cleaned_continuum_regions.append(
            (wave_base, wave_top, num_measures, mean_flux, std_flux)
        )
...
def generate_random_continuum(waveobs, nknots=None):
    ...
    half_wrange = wrange / 2.0
    if nknots is None:
        nknots = np.max([1, int(wrange / 5.0)])
    ...
    y *= 1 + ((1 + np.sin(2 * np.pi * f * x / fs)) / 2.0) * 0.1
