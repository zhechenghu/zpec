...
#    Modified from original ispec file by Zhecheng Hu.
#    1. There's no optimized version, only pure python slow version.
import numpy as np
...
def __read_fits_spectrum(spectrum_filename):
    ...
    if data is not None and (
        type(hdulist["PRIMARY"]) is pyfits.hdu.image.PrimaryHDU
        or type(hdulist["PRIMARY"]) is pyfits.hdu.image.ImageHDU
    ):
        axis = 1
        specaxis = str(axis)
        ...
        if hdr.get(str("CD%s_%s" % (specaxis, specaxis))) is not None:
            ...
            logging.info(
                "Using the FITS CD matrix.  PIX=%f VAL=%f DELT=%f UNIT=%s"
                % (reference_pixel, wave_base, wave_step, unit)
            )
            ...
            logging.info(
                "No CDELT or CD in header.  Assuming 2D input with 1st line representing the spectral axis."
            )
        ...
        if waveobs is None:
            if hdr.get("WFITTYPE") == "LOG-LINEAR":
                xconv = lambda v: 10 ** (
                    (v - reference_pixel + 1) * wave_step + wave_base
                )
        ...
        for i in range(len(hdulist)):
            ...
            if (
                name == str("PRIMARY")
                or len(hdulist[i].data.flatten()) != len(flux)
                or type(hdulist[i]) is pyfits.hdu.table.BinTableHDU
            ):
                continue
            if "IVAR" in name or "IVARIANCE" in name:
                spectrum["err"] = np.sqrt(1.0 / hdulist[i].data.flatten())  # Not sure
...
def __read_spectrum(spectrum_filename):
    try:
        ...
        try:
            ...
            spectrum = np.array(
                [tuple(line.rstrip("\r").split()) for line in narval[2:-1]],
                dtype=[("waveobs", float), ("flux", float), ("err", float)],
            )
            if len(spectrum) == 0:
                raise Exception("Empty spectrum or incompatible format")
            ...
            spectrum = np.array(
                [tuple(line.rstrip("\r").split()) for line in espadons[2:]],
                dtype=[("waveobs", float), ("flux", float), ("err", float)],
            )
...
def read_spectrum(spectrum_filename, apply_filters=True, sort=True, regions=None):
    ...
    ** It can recognise FITS files by the filename (extensions .FITS or .FIT), if this is
    ...
    if os.path.exists(spectrum_filename) and (
        spectrum_filename[-4:].lower() == ".fit"
        or spectrum_filename[-5:].lower() == ".fits"
        or spectrum_filename[-7:].lower() == ".fit.gz"
        or spectrum_filename[-8:].lower() == ".fits.gz"
    ):
        ...
        os.path.exists(spectrum_filename) and spectrum_filename[-3:].lower() == ".gz"
    ) or (os.path.exists(spectrum_filename.lower() + ".gz")):
    ...
    if (
        ...
        if np.all(np.abs(wave_diff - median_wave_step) < 0.0000001):
            ...
            if np.any(spectrum["err"] != 0):
                ...
                extension_hdu = pyfits.ImageHDU(
                    data=ext_data, header=extheader, name="SIGMA"
                )
        ...
        out.write(
            "\n".join(
                [
                    "\t".join(map(str, (line["waveobs"], line["flux"], line["err"])))
...
def normalize_spectrum(spectrum, continuum_model, consider_continuum_errors=True):
    ...
    if np.all(continuum_flux == 1) and (
        not consider_continuum_errors or np.all(continuum_errors == 0)
    ):
        return spectrum.copy()

    normalized_spectrum = create_spectrum_structure(spectrum["waveobs"])
    zeros = continuum_flux == 0
    normalized_spectrum["flux"][zeros] = 1.0
    normalized_spectrum["err"][zeros] = 0.0
    normalized_spectrum["flux"][~zeros] = (
        spectrum["flux"][~zeros] / continuum_flux[~zeros]
    )

    # Error propagation considering errors in continuum (most conservative operation)
    if (consider_continuum_errors and np.all(continuum_errors == 0)) or np.all(
        spectrum["err"] == 0
    ):
        normalized_spectrum["err"] = 0.0
    else:
        if consider_continuum_errors:
            zeros = np.logical_or(spectrum["flux"] == 0, continuum_flux == 0)
            normalized_spectrum["err"][~zeros] = normalized_spectrum["flux"][~zeros] * (
                (spectrum["err"][~zeros] / spectrum["flux"][~zeros])
                + (continuum_errors[~zeros] / continuum_flux[~zeros])
            )
            ...
            normalized_spectrum["err"][~zeros] = (
                spectrum["err"][~zeros] / continuum_flux[~zeros]
            )
...
def __interpolation(
    waveobs, fluxes, err, resampled_waveobs, bessel=False, zero_edges=True, frame=None
):
    """
        ...
        # resampled_err[i] = err[index]
            ...
            if not bessel or index == 1 or index == total_points - 1:
                ...
                if fluxes[index - 1] <= 1e-10 or fluxes[index] <= 1e-10:
                    ...
                    resampled_flux[i] = fluxes[index - 1] + d1 * (
                        (fluxes[index] - fluxes[index - 1]) / d2
                    )
                    ...
                    resampled_err[i] = (
                        err[index - 1] * (d2 - d1) + (err[index] * d1)
                    ) / d2
                ...
                if (
                    ...
                    resampled_flux[i] = (
                        flux_x0
                        + p * (flux_x1 - flux_x0)
                        + factor * (flux_x2 - flux_x1 - flux_x0 + flux_x_1)
                    )
                    # Same formula as for interpolation but I have re-arranged the terms to make
                    # clear that it is valid for error propagation (sum of errors multiplied by constant values)
                    resampled_err[i] = (
...
def __convolve_spectrum_slow(
    waveobs, flux, err, to_resolution, from_resolution=None, frame=None
):
    """
    ...
    for i in np.arange(total_points):
        ...
        gaussian = np.exp(
            -((waveobs_window - lambda_peak) ** 2) / (2 * sigma[i] ** 2)
        ) / np.sqrt(2 * np.pi * sigma[i] ** 2)
...
def __convolve_spectrum(
    ...
    for i in np.arange(len(nbins)):
        ...
        gaussian = np.exp(
            -((waveobs_segment - current_center) ** 2) / (2 * current_sigma**2)
...
def convolve_spectrum(spectrum, to_resolution, from_resolution=None, frame=None):
    ...
    waveobs, flux, err = __convolve_spectrum(
        spectrum["waveobs"],
        spectrum["flux"],
        spectrum["err"],
        to_resolution,
...
def create_spectrum_structure(waveobs, flux=None, err=None):
    ...
    spectrum = np.recarray(
        (len(waveobs),), dtype=[("waveobs", float), ("flux", float), ("err", float)]
    )
...
def resample_spectrum(spectrum, xaxis, method="linear", zero_edges=True, frame=None):
    ...
    if method.lower() == "linear":
        ...
        waveobs, flux, err = __interpolation(
            spectrum["waveobs"],
            spectrum["flux"],
            spectrum["err"],
            xaxis,
        ...
        f = interpolate.InterpolatedUnivariateSpline(
            spectrum["waveobs"], spectrum["flux"], k=3
        )
        e = interpolate.InterpolatedUnivariateSpline(
            spectrum["waveobs"], spectrum["err"], k=3
        )
        flux = f(xaxis)
        err = e(xaxis)
        current_work_progress = 90.0
...
def correct_velocity(spectrum, velocity, segments=None):
    ...
    corrected_spectrum = create_spectrum_structure(
        spectrum["waveobs"], spectrum["flux"], spectrum["err"]
    )
    # - Newtonian version:
    ...
    corrected_spectrum["waveobs"] = spectrum["waveobs"] * np.sqrt(
        (1.0 - (velocity * 1000.0) / c) / (1.0 + (velocity * 1000.0) / c)
    )
...
def add_noise(spectrum, snr, distribution="poisson"):
    ...
    """
    noisy_spectrum = create_spectrum_structure(
        spectrum["waveobs"], spectrum["flux"], spectrum["err"]
    )
...
def random_realizations(spectrum, number, distribution="poisson"):
    ...
    for i in range(number):
        new_derived_spectrum = create_spectrum_structure(
            spectrum["waveobs"], spectrum["flux"], spectrum["err"]
        )
        if distribution.lower() == "gaussian":
            sigma = spectrum["err"]
...
def create_wavelength_filter(spectrum, wave_base=None, wave_top=None, regions=None):
    ...
    if regions is None:
        ...
        wfilter = (
            spectrum["waveobs"] == np.min(spectrum["waveobs"]) - 1.0
        )  # Make all false
    else:
        ...
        for region in regions:
            ...
            if wfilter is None:
                wfilter = np.logical_and(
                    spectrum["waveobs"] >= wave_base, spectrum["waveobs"] <= wave_top
                )
            else:
                wfilter = np.logical_or(
                    wfilter,
                    np.logical_and(
                        spectrum["waveobs"] >= wave_base,
...
def air_to_vacuum(spectrum):
    ...
    wave_air = spectrum["waveobs"] * 10.0  # Angstroms
    s_square = np.power(1.0e4 / wave_air, 2)
    n2 = (
        1.0
        + 0.00008336624212083
        + 0.02408926869968 / (130.1065924522 - s_square)
        + 0.0001599740894897 / (38.92568793293 - s_square)
    )
    wave_vacuum = wave_air * n2  # Angstroms
    converted_spectrum = create_spectrum_structure(
        wave_vacuum / 10.0, spectrum["flux"], spectrum["err"]
    )
...
def vacuum_to_air(spectrum):
    ...
    wave_vacuum = spectrum["waveobs"] * 10.0  # Angstroms
    s_square = np.power(1.0e4 / wave_vacuum, 2)
    n = (
        1
        + 0.0000834254
        + 0.02406147 / (130 - s_square)
        + 0.00015998 / (38.9 - s_square)
    )
    wave_air = wave_vacuum / n  # Angstroms
    converted_spectrum = create_spectrum_structure(
        wave_air / 10.0, spectrum["flux"], spectrum["err"]
    )

    return converted_spectrum


def create_filter_cosmic_rays(
    spectrum,
    continuum_model,
    resampling_wave_step=0.001,
    window_size=15,
    ...
    wavelengths = np.arange(
        ...
        np.max(spectrum["waveobs"]) + resampling_wave_step,
        resampling_wave_step,
    )
    ...
    resampled_smooth["flux"] = scipy.signal.medfilt(
        resampled_spectrum["flux"], window_size
    )
    smooth = resample_spectrum(resampled_smooth, spectrum["waveobs"])

    cosmics = (spectrum["flux"] - smooth["flux"]) / continuum_model(
        spectrum["waveobs"]
    ) > variation_limit
    cosmics = np.logical_and(
        cosmics, (spectrum["flux"] / continuum_model(spectrum["waveobs"])) > 1.0
    )
    return cosmics
